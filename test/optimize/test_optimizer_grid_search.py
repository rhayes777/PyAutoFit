...
def make_grid_search(mapper):
    return gs.GridSearch(phase_path='', phase_name='', model_mapper=mapper, number_of_steps=10)
...
class TestGridSearchablePriors(object):
    ...
    def test_non_grid_searched_dimensions(self, mapper):
        grid_search = gs.GridSearch(phase_path='', phase_name='', model_mapper=mapper, number_of_steps=10)
...
class MockClassContainer(object):
    def __init__(self):
        ...
        class MockOptimizer(non_linear.NonLinearOptimizer):
            def __init__(self, phase_path='', phase_name="mock_optimizer", model_mapper=None):
...
def make_grid_search_05(mapper, container):
    return gs.GridSearch(model_mapper=mapper, optimizer_class=container.MockOptimizer, number_of_steps=2,
                         phase_path='', phase_name="sample_name")
...
class TestMixin(object):
    def test_mixin(self, container):
        ...
        optimizer = MyPhase(phase_path='', phase_name='', number_of_steps=2, optimizer_class=container.MockOptimizer)
