...
def make_grid_search(mapper, parent_search):
    search = _make_grid_search(mapper, parent_search)
    search.paths.save_all()
    return search


@pytest.fixture(name="database_grid_search")
def make_database_grid_search(mapper, database_parent_search, session):
    return _make_grid_search(mapper, database_parent_search, session=session)


class TestMiscombination:
    def test_directory_for_database(self, parent_search, session, mapper):
        with pytest.raises(TypeError):
            _make_grid_search(mapper, parent_search, session)


class TestDirectory:
    def test_parent_search(self, grid_search, parent_search):
        grid_paths = grid_search.paths
        parent_paths = parent_search.paths

        assert parent_paths is grid_paths.parent
        with open(grid_paths._parent_identifier_path) as f:
...
class TestDatabase:
    def test_parent_search(self, database_grid_search, database_parent_search):
