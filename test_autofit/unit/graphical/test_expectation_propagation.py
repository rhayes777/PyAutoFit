...
def make_model_approx(model, x):
    return mp.MeanFieldApproximation.from_kws(
        model, {x: autofit.graphical.messages.normal.NormalMessage(0, 1)}
    )
...
def test_approximations(probit_approx, model_approx, x):
    ...
    probit_model = autofit.graphical.messages.normal.NormalMessage.from_mode(
        result.mode[x], covariance=result.inv_hessian[x]
    )
...
def test_looped_importance_sampling(model, normal_factor, probit_factor, x):
    model_approx = mp.MeanFieldApproximation.from_kws(
        model, {x: autofit.graphical.messages.normal.NormalMessage(0, 1)}
    )

    np.random.seed(1)
    sampler = mp.ImportanceSampler(n_samples=1000)
    history = list()
    ...
    for i in range(3):
        for factor in [normal_factor, probit_factor]:
            ...
            model_dist = mp.project_factor_approx_sample(factor_approx, sample)

            # divide projection by cavity distribution
            factor_project, status = factor_approx.project(model_dist, delta=1)

            # update model approximation
            model_approx, status = model_approx.project(factor_project, status)
