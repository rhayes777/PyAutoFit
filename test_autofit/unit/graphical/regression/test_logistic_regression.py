...
def make_model(prior_a, prior_b, likelihood_factor, linear_factor):
...
def make_model_approx(model, a_, b_, z_, x_, y_):
    ...
    return mp.MeanFieldApproximation.from_kws(
        model,
        {
            a_: mp.NormalMessage.from_mode(np.zeros((n_features, n_dims)), 10),
            b_: mp.NormalMessage.from_mode(np.zeros(n_dims), 10),
            z_: mp.NormalMessage.from_mode(np.zeros((n_obs, n_dims)), 10),
