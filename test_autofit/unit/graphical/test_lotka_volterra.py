...
def test():
    ...
    A = np.array([[0.0, beta / alpha], [delta / gamma, 0.0]])
    K = 1
    noise = 0.1
    ...
    t_space = 1.0
    t_obs = np.r_[
        0, (np.arange(n_obs - 1) * t_space + np.random.rand(n_obs - 1)) * t_space
    ]
    ...
    def calc_lotka_volterra(y0, r, A, K, t_obs):
        ...
        if n != n_obs:
            y_ = np.c_[y_, np.repeat(y_[:, [-1]], n_obs - n, axis=1)][:, :n_obs]
            if y_.shape[1] != n_obs:
                raise Exception
    ...
    model_approx = mp.MeanFieldApproximation.from_kws(
        LV_model,
        {
            A_: autofit.graphical.messages.normal.NormalMessage.from_mode(A, 100.0),
            r_: autofit.graphical.messages.normal.NormalMessage.from_mode(r, 100.0),
            y0_: autofit.graphical.messages.gamma.GammaMessage.from_mode(
                np.ones_like(y0), 1
            ),
    ...
    for i in range(n_iter):
        ...
        for factor in factors:
            model_approx, status = opt.laplace_factor_approx(model_approx, factor)
