...
def test_is_database_paths(session):
    optimizer = af.m.MockOptimizer(session=session)
    assert isinstance(optimizer.paths, af.DatabasePaths)
...
def test_save_all_samples_boolean(session, save_all_samples):
    optimizer = af.m.MockOptimizer(session=session, save_all_samples=save_all_samples)
    # noinspection PyUnresolvedReferences
...
def test_unique_tag(session):
    ...
    optimizer = af.m.MockOptimizer(session=session, unique_tag=unique_tag)

    assert optimizer.paths.unique_tag == unique_tag

    optimizer.fit(model, analysis)
