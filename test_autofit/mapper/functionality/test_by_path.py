...
class TestInstanceFromPathArguments:
    ...
    def test_prior_linking(self, model):
...
class TestInstanceFromPriorNames:
    ...
    def test_prior_linking(self, underscore_model):
        ...
        instance = underscore_model.instance_from_prior_name_arguments(
            {"gaussian_component_centre": 0.1, "gaussian_component_sigma": 0.3}
        )
        assert instance.gaussian_component.centre == 0.1
        ...
        instance = underscore_model.instance_from_prior_name_arguments(
            {"gaussian_component_normalization": 0.2, "gaussian_component_sigma": 0.3}
        )
    ...
    def test_path_for_name(self, underscore_model):
        assert underscore_model.path_for_name("gaussian_component_centre") == (
            "gaussian_component",
            "centre",
        )
...
def test_with_tuple():
    with_tuple = af.Model(af.m.MockWithTuple)
    assert with_tuple.model_component_and_parameter_names == ["tup_0", "tup_1"]
