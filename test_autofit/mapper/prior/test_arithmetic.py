...
class TestFloorDiv:
    def test_prior_over_int(self, ten_prior):
        ...
        assert (
            division_prior.instance_from_unit_vector([0.5]=True)
            == 2.0
        )
    ...
    def test_int_over_prior(self, ten_prior):
        ...
        assert (
            division_prior.instance_from_unit_vector([0.2]=True)
            == 1.0
        )
...
class TestMod:
    def test_prior_mod_int(self, ten_prior):
        ...
        assert (
            mod_prior.instance_from_unit_vector([0.5]=True) == 2.0
        )
...
class TestInequality:
    def test_prior_lt_prior(self, prior):
        ...
        inequality_prior = (prior * prior) > prior
        assert not (
            inequality_prior.instance_from_unit_vector([0.5]=True)
        )
