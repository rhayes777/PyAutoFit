...
def numerical_func_jacobian(
    ...
    if _calc_deterministic:
        ...
        det_slices = {v: (slice(None),) * np.ndim(a) for v, a in det_vars0.items()}
...
def numerical_func_jacobian_hessian(
    ...
    fhess0 = JacobianValue(
        {v: np.empty(f_shape + np.shape(values[v]) * 2) for v in variables}
    )
        ...
        if x0.shape:
            ...
            if _calc_deterministic:
                ...
                if f_shape:
                    ...
                    for d, d_shape in det_shapes.items():
                        jac = fjac0[v][d].reshape(np.prod(d_shape), var_size)
                        hess_d = np.einsum("ij,ljk,mk->lim", jac, hess2d, jac)
                        fhess0[d] += hess_d.reshape(d_shape + d_shape + f_shape)
                else:
                    hess2d = hess.reshape((var_size, var_size))
                    for d, d_shape in det_shapes.items():
                        jac = fjac0[v][d].reshape(np.prod(d_shape), var_size)
                        hess_d = np.linalg.multi_dot([jac, hess2d, jac.T])
            ...
            if _calc_deterministic:
                ...
                for d, d_shape in det_shapes.items():
                    ...
                    fhess0[d] += (
                        jac[:, None, None] * jac[None, :, None] * hess[None, None, :]
                    )
